(function(root, factory){
 ...
 function additionSpots(total,split){
  ...
  for(var i=0;i<split;i++)spots.push([-2.1+(i%2)*.55,1.16,.2+Math.floor(i/2)*.5]);
  for(var j=0;j<rightCount;j++)spots.push([.05+(j%2)*.55,1.16,.2+Math.floor(j/2)*.5]);
  return spots;
 }
